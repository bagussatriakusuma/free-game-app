...
import com.example.challengechapter7.presentation.main.home.adapter.PopularMobaGameAdapter
import com.example.challengechapter7.presentation.main.home.adapter.PopularRacingGameAdapter
import com.example.challengechapter7.presentation.main.home.adapter.RecommendedGameAdapter
...
class HomeFragment : Fragment() {
    ...
    private fun bindAdapter(){
        recommendedGameAdapter = RecommendedGameAdapter(object : RecommendedGameAdapter.OnClickListener {
            override fun onClickItem(data: AllGames) {
                val bundle = Bundle()
    ...
    private fun handleDataRecommendedGames(recommendedGames: List<AllGames>) {
        recommendedGameAdapter.submitData(recommendedGames)
    }

    private fun handleDataPopularMobaGames(popularMobaGames: List<AllGames>) {
        popularMobaGameAdapter.submitData(popularMobaGames)
    }

    private fun handleDataPopularRacingGames(popularRacingGames: List<AllGames>) {
